...
pub fn print_layers() {
    match instance::layers_list() {
        ...
        Err(err) => println!("Error layers_list(): {}", err),
    }
}

pub fn print_extensions(ext: InstanceExtensions) {
    let list = [
        ("khr_surface", ext.khr_surface),
        ("khr_display", ext.khr_display),
        ("khr_xlib_surface", ext.khr_xlib_surface),
        ("khr_xcb_surface", ext.khr_xcb_surface),
        ("khr_wayland_surface", ext.khr_wayland_surface),
        ("khr_android_surface", ext.khr_android_surface),
        ("khr_win32_surface", ext.khr_win32_surface),
        ("ext_debug_utils", ext.ext_debug_utils),
        ("mvk_ios_surface", ext.mvk_ios_surface),
        ("mvk_macos_surface", ext.mvk_macos_surface),
        ("mvk_moltenvk", ext.mvk_moltenvk),
        ("nn_vi_surface", ext.nn_vi_surface),
        ("ext_swapchain_colorspace", ext.ext_swapchain_colorspace),
        (
        ...
        (
            ...
            ext.khr_get_surface_capabilities2,
        ),
    ];
    for (name, ok) in list.iter() {
        if *ok {
            println!("{}", name)
        }
    }
...
pub fn print_pd(pd: &PhysicalDevice) {
    ...
    println!("  id    local    visible   coherent   cached    lazy   size");
